...
static void on_fs_start_playback(medhub_context_t *ctx, const nlohmann::json &hub_event);
...
static bool pause_current_playing_for(medhub_context_t *ctx, switch_core_session_t *session, int32_t *playing_duration);
static bool resume_current_playing_for(medhub_context_t *ctx, switch_core_session_t *session, int32_t *playing_duration);
...
public:
    ...
    void on_message(websocketpp::connection_hdl hdl, message_ptr msg) {
        ...
        switch (msg->get_opcode()) {
            case websocketpp::frame::opcode::text: {
                ...
                } else if (hubevent["header"]["name"] == "FSStartPlayback") {
                    on_fs_start_playback(_medhub_ctx, hubevent);
                } else if (hubevent["header"]["name"] == "FSStopPlayback") {
                    on_fs_stop_playback(_medhub_ctx, hubevent);
                } else if (hubevent["header"]["name"] == "FSPausePlayback") {
    ...
    void send_playback_paused_event(const char *playback_id, int32_t playing_duration) {
        if (medhub_globals->_debug) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[%s]: send playback paused event\n",
                              _medhub_ctx->sessionid);
        }

        if (!is_playback_connected()) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "[%s]: send_playback_paused_event failed for playback_not_connected!\n",
                              _medhub_ctx->sessionid);
            return;
        }
        ...
        const std::string str_event = json_event.dump();
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "[%s]: send_playback_paused_event: detail: %s\n",
                          _medhub_ctx->sessionid, str_event.c_str());
    ...
    void send_playback_resumed_event(const char *playback_id, int32_t playing_duration) {
        if (medhub_globals->_debug) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[%s]: send playback resumed event\n",
                              _medhub_ctx->sessionid);
        }

        if (!is_playback_connected()) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "[%s]: send_playback_resumed_event failed for playback_not_connected!\n",
                              _medhub_ctx->sessionid);
            return;
        }
...
void on_transcription_result_changed(medhub_context_t *ctx, const nlohmann::json &hub_event) {
    ...
    if (is_playing(ctx) && !is_paused(ctx)) {
        pause_current_playing_for(ctx, ctx->session, nullptr);
        if (medhub_globals->_debug) {
            std::string id_str = get_thread_id(std::this_thread::get_id());
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE,
...
static void notify_playback_stop(const char *session_uuid, const char *playback_id) {
    switch_core_session *session  = switch_core_session_force_locate(session_uuid);
    if (!session) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "notify_playback_stop: locate session [%s] failed, maybe ended\n",
                          session_uuid);
        return;
    }
    ...
    if (!ctx) {
        switch_core_session_rwunlock(session);
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "notify_playback_stop: can't found medhub ctx by %s\n", session_uuid);
        return;
    }
    ...
    if (switch_atomic_read(&ctx->asr_stopped) == 1) {
        ...
        switch_mutex_lock(ctx->mutex);
        if (ctx->client) {
            ctx->client->send_playback_stopped_event(playback_id);
        }
        switch_mutex_unlock(ctx->mutex);
    }
    switch_core_session_rwunlock(session);
...
static void on_playback_stop(switch_event_t *event) {
    ...
    if (uuid && playback_id) {
        notify_playback_stop(uuid, playback_id);
    }
...
static bool pause_current_playing_for(medhub_context_t *ctx, switch_core_session_t *session, int32_t *playing_duration) {
    ...
    if (SWITCH_STATUS_SUCCESS == status) {
        if (!switch_test_flag(fhp, SWITCH_FILE_PAUSE)) {
            ...
            switch_core_file_command(fhp, SCFC_PAUSE_READ);
            ctx->is_paused = true;
            // current_playing_duration(session);
            if (playing_duration) {
        ...
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                          "[%s]: pause_current_playing_for: failed for switch_ivr_get_file_handle return %d\n",
    ...
    if (!session4play) {
        ...
        if (!ctx) {
            ...
            if (is_playing(ctx) && is_paused(ctx)) {
                ...
                resume_current_playing_for(ctx, ctx->session, nullptr);
...
static void on_fs_start_playback(medhub_context_t *ctx, const nlohmann::json &hub_event) {
    ...
    if (medhub_globals->_debug) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "on_fs_start_playback: uuid[%s], content_id[%s], file[%s]\n",
                          uuid.c_str(), content_id.c_str(), file.c_str());
    }

    switch_core_session_t *session = switch_core_session_force_locate(uuid.c_str());
    if (!session) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "on_fs_start_playback failed, can't found session by %s\n", uuid.c_str());
    } else {
        switch_mutex_lock(ctx->mutex);

        ctx->content_id = switch_core_session_strdup(session, content_id.c_str());
        ctx->playback_file = switch_core_session_strdup(session, file.c_str());
        ctx->playback_idx++;
        ...
        //switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "[%s] on_fs_start_playback: %s\n", uuid.c_str(), file.c_str());
        {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
                              "on_fs_start_playback: %s before switch_ivr_broadcast: %s\n",
                              uuid.c_str(), file.c_str());
            switch_ivr_broadcast(uuid.c_str(), file.c_str(), (SMF_NONE | SMF_ECHO_ALEG | SMF_ECHO_BLEG));
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
                              "on_fs_start_playback: %s after switch_ivr_broadcast %s\n",
                              uuid.c_str(), file.c_str());
        }
    }
}
...
static void on_fs_stop_playback(medhub_context_t *ctx, const nlohmann::json &hub_event) {
    ...
    if (medhub_globals->_debug) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "on_fs_stop_playback: uuid[%s], playback_id[%s]\n",
                          uuid.c_str(), playback_id.c_str());
    }

    switch_core_session_t *session = switch_core_session_force_locate(uuid.c_str());
    if (!session) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "on_fs_stop_playback failed, can't found session by %s\n", uuid.c_str());
    } else {
        switch_mutex_lock(ctx->mutex);
...
static void on_fs_pause_playback(medhub_context_t *ctx, const nlohmann::json &hub_event) {
    ...
    switch_core_session_t *session = switch_core_session_force_locate(uuid.c_str());
    if (!session) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "on_fs_pause_playback failed, can't found session by %s\n", uuid.c_str());
    } else {
        int32_t playing_duration = -1;
        switch_mutex_lock(ctx->mutex);
        if (pause_current_playing_for(ctx, session, &playing_duration)) {
            if (ctx->client) {
                ctx->client->send_playback_paused_event(playback_id.c_str(), playing_duration);
            }
        }
        switch_mutex_unlock(ctx->mutex);
        switch_core_session_rwunlock(session);
...
static void on_fs_resume_playback(medhub_context_t *ctx, const nlohmann::json &hub_event) {
    ...
    switch_core_session_t *session = switch_core_session_force_locate(uuid.c_str());
    if (!session) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "on_fs_resume_playback failed, can't found session by %s\n", uuid.c_str());
    } else {
        int32_t playing_duration = -1;
        switch_mutex_lock(ctx->mutex);
        if (resume_current_playing_for(ctx, session, &playing_duration)) {
            if (ctx->client) {
                ctx->client->send_playback_resumed_event(playback_id.c_str(), playing_duration);
            }
        }
        switch_mutex_unlock(ctx->mutex);
        switch_core_session_rwunlock(session);
